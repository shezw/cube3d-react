...
| --- | --- | --- | --- | --- |
| Anchor orientation | `anchor-orientation` | `anchor-orientation.test.ts` checks attachment orientation across rotated anchors | The page lays out three small comparison tiles at once. Each tile independently renders one state: `positionOnlyControl`, `orientationAttach`, or `orientationWithParentTransform`. Tests assert each tile has its own WebGL/Cube3D pair, helper plane/axis guides, expectation text, PASS result, matching template geometry, and only the intended variable changes | The library can place a part but cannot prove it is facing the correct way under rotation |
| Pivot / origin | `pivot-origin` | `pivot.test.ts` checks center, left-hinge, and top-hinge pivots; each declared pivot point must remain fixed under rotation | The page lays out three small comparison tiles at once: `centerPivotCase`, `leftHingeCase`, and `topHingeCase`. Tests assert each tile has pivot plane/axis/pin guides, explicit expected behavior, PASS result, and the door/base/handle template is identical except `pivot` | Visual rotation may be happening around an implicit CSS center instead of the modeled origin |
| World bounds / spatial query | `world-bounds` | `world-query.test.ts` checks translated, rotated, and nested scaled bounds; root bounds must contain every subcase | The page lays out three small comparison tiles at once: `translatedStack`, `rotatedStack`, and `nestedScaledStack`. Tests assert each tile has footprint and local-axis guides, explicit expected behavior, PASS result, and the stack geometry/material/local child transforms are identical while only the transform context changes | Objects may be visible, but the model layer cannot reliably query or reason about the scene after transforms |
