...
| --- | --- | --- | --- | --- |
| Anchor orientation | `anchor-orientation` | `anchor-orientation.test.ts` checks attachment orientation across rotated anchors | The demo is a controlled comparison of the same socket/plug template: `positionOnlyControl`, `orientationAttach`, and `orientationWithParentTransform`. Tests assert geometry, colors, transforms, and anchors are identical except attachment mode and parent transform condition. Position-only must align position but fail normal alignment; orientation cases must align both position and orientation | The library can place a part but cannot prove it is facing the correct way under rotation |
| Pivot / origin | `pivot-origin` | `pivot.test.ts` checks center, left-hinge, and top-hinge pivots; each declared pivot point must remain fixed under rotation | The demo uses the same door/base/handle template in `centerPivotCase`, `leftHingeCase`, and `topHingeCase`. Tests assert the door template is identical except `pivot`, DOM exposes `data-cube3d-pivot`, and the handle stays attached after rotation | Visual rotation may be happening around an implicit CSS center instead of the modeled origin |
| World bounds / spatial query | `world-bounds` | `world-query.test.ts` checks translated, rotated, and nested scaled bounds; root bounds must contain every subcase | The demo uses the same stack template in `translatedStack`, `rotatedStack`, and `nestedScaledStack/innerStack`. Tests assert stack geometry/material/local child transforms are identical and only the transform context changes; root core bounds contain each subcase bounds | Objects may be visible, but the model layer cannot reliably query or reason about the scene after transforms |

These checks deliberately combine semantic tests and browser tests. A screenshot that looks acceptable is not enough: the model must expose stable paths, anchors, pivots, and bounds that a real project can query. Spatial comparison demos must be controlled comparisons: the same object template is reused, and tests must prove which field is the experimental variable.
