...
| --- | --- | --- | --- | --- |
| Anchor orientation | `anchor-orientation` | `anchor-orientation.test.ts` now checks straight, rotated, and counter-rotated attachment cases; each case must align anchor position, `normal`, and `tangent` | The demo contains `straightCase`, `rotatedCase`, and `parentRotatedCase`; DOM exposes `data-cube3d-anchor-normal/tangent`; each case checks projected `socket.out` and `plug.in` distance | The library can place a part but cannot prove it is facing the correct way under rotation |
| Pivot / origin | `pivot-origin` | `pivot.test.ts` now checks center, left-hinge, and top-hinge pivots; each declared pivot point must remain fixed under rotation | The demo contains `centerPivotCase`, `leftHingeCase`, and `topHingeCase`; DOM exposes `data-cube3d-pivot`, a pivot marker, and Chromium `transform-origin` for each door; handle anchors must stay attached after rotation | Visual rotation may be happening around an implicit CSS center instead of the modeled origin |
| World bounds / spatial query | `world-bounds` | `world-query.test.ts` now checks translated, rotated, and nested scaled bounds; root bounds must contain every subcase | Browser verifies translated, rotated, nested scaled, and outer marker paths exist; repeated models are counted; root core bounds contain each subcase bounds | Objects may be visible, but the model layer cannot reliably query or reason about the scene after transforms |
